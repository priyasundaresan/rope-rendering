...
class RopeRenderer:
    def __init__(self, rope_radius=None, sphere_radius=None, rope_iterations=None, rope_screw_offset=None, bezier_scale=3.7, bezier_knots=12, save_depth=True, save_rgb=False):
        """
        ...
        #self.bezier_scale = 3.2 # was this for sim exp
        self.bezier_scale = 2.8
    ...
    def add_camera(self, fixed=True):
        ...
        if fixed:
            ...
            self.camera.rotation_euler = (0, 0, random.uniform(-pi, pi)) # fixed z, rotate only about x/y axis
...
#        bpy.ops.transform.resize(value=(0.45, 0.45, 0.45))
    ...
    def reposition_camera(self):
        ...
        #self.camera.location.z += np.random.uniform(3.3, 3.6) # FOR SIM EXPS
        self.camera.location.z += np.random.uniform(3.8, 3.9)
    ...
    def render_single_scene(self, M_pix=20, M_depth=0.2):
        ...
        coords = [rope_deformed.matrix_world @ v.co for v in list(rope_deformed.data.vertices)[::45]] # TODO: this is actually where i specify how many vertices to export (play around with :20); will standardize this
