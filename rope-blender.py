...
class RopeRenderer:
    def __init__(self, rope_radius=0.1, rope_screw_offset=8, rope_length=10, bezier_scale=7, bezier_subdivisions=10):
    ...
    def make_rigid_rope(self):
        ...
        self.rope.modifiers["Screw"].iterations = self.rope_length
    ...
    def add_rope_asymmetry(self):
        ...
        bpy.ops.transform.rotate(value= pi / 2, orient_axis='X')
        bpy.ops.mesh.primitive_ico_sphere_add(radius=0.5, location=(self.origin[0], self.origin[1], self.origin[2] + self.rope_radius*self.rope_screw_offset*self.rope_length))
        bpy.ops.object.select_all(action='SELECT')
        bpy.ops.object.join()
